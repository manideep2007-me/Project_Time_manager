...
export default function AdminProjectDetailsScreen() {
    ...
    try {
      ...
      try {
        ...
        const teamMembersData = teamData.map((member: any) => {
          ...
          const minutes = statsForMember ? (statsForMember.totalMinutes || 0) : 0;
          return {
            id: member.id,
  ...
  // Format minutes to HH:MM:SS for logged time
  const formatHMS = (totalMinutes: number) => {
    if (!totalMinutes || totalMinutes < 0) totalMinutes = 0;
    const hours = Math.floor(totalMinutes / 60);
    const minutes = Math.floor(totalMinutes % 60);
    const seconds = 0; // Only minutes granularity available
              ...
              <View style={{ gap: 10 }}>
                {teamMembers.length > 0 ? (
                  teamMembers.map((member, index) => {
                    const initials = (member.name || 'T M').split(' ').map((n: string) => n.charAt(0)).slice(0,2).join('').toUpperCase();
                    return (
                      <View key={index} style={styles.teamItem}>
                        ...
                        <View style={{ flex: 1 }}>
                          ...
                          <Text style={styles.memberRole}>{member.department || member.role || 'Member'}</Text>
                        </View>
                        <View style={{ alignItems: 'flex-end' }}>
                          <TouchableOpacity
                            accessibilityRole="button"
                            accessibilityLabel={`Remove ${member.name} from project`}
                            onPress={() => handleRemoveMember(member.id || '', member.name || 'Unknown')}
                            style={styles.removeBtn}
                          >
                            <Text style={styles.removeBtnText}>×</Text>
                          </TouchableOpacity>
                          <Text style={styles.loggedLabel}>Logged Time</Text>
                          <Text style={styles.loggedValue}>{formatHMS(member.totalMinutes || Math.round((member.hours || 0)*60))}</Text>
